...
def remove_jsp_comments(text):
    """JSP (.jsp) and JSP fragments (.jspf) mix three independent comment
    layers, so a plain HTML or Java handler alone gets it wrong:
      - '<%-- ... --%>' JSP comments: removed at translation time, never
        reach the client, and may span multiple lines - always safe to strip.
        Checked FIRST since '<%--' is a more specific prefix than '<%'.
      - '<% %>' scriptlets, '<%! %>' declarations, '<%= %>' expressions and
        '<%@ %>' directives contain plain Java (or Java-like attribute
        values), so '//' and '/* */' inside them are stripped with the same
        string/char-literal-aware scanner as remove_java_comments - reused
        directly rather than re-implemented, so text blocks/escapes behave
        identically. The tag boundary itself ('%>' ending the tag) is found
        with a small quote-aware scan first, so a '%>' that happens to sit
        inside a Java string/char literal never ends the tag early.
      - '<!-- ... -->' literal HTML comments in the template portion pass
        through to the client like plain HTML, so they are stripped the
        same way remove_html_comments/remove_xml_comments treat them.
    """
    output, index, length = [], 0, len(text)
    while index < length:
        if text.startswith("<%--", index):
            index += 4
            while index < length:
                if text.startswith("--%>", index): index += 4; break
                if text[index] in "\r\n": output.append(text[index])
                index += 1
            continue
        if text.startswith("<%", index):
            open_len = 3 if text[index:index + 3] in ("<%!", "<%=", "<%@") else 2
            start, cursor = index, index + open_len
            ...
            while cursor < length:
                ...
                if quote:
                    ...
                    cursor += 1; continue
                if char in ('"', "'"):
                    quote = char; cursor += 1; continue
                if text.startswith("%>", cursor):
                    cursor += 2; break
                cursor += 1
            output.append(remove_java_comments(text[start:cursor]))
            index = cursor
            continue
        if text.startswith("<!--", index):
            index += 4
...
HANDLERS.update({".html": remove_html_comments, ".htm": remove_html_comments, ".php": remove_php_comments, ".jsp": remove_jsp_comments, ".jspf": remove_jsp_comments, ".sql": remove_sql_comments, ".xml": remove_xml_comments, ".xaml": remove_xml_comments, ".svg": remove_xml_comments, ".lua": remove_lua_comments, ".svelte": remove_html_comments})
# ---------------------------------------------------------------------------
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            ...
            HANDLERS/GENERIC_CONFIGS in the source for the full extension list):
              ...
              VB.NET/classic BASIC, Julia, PHP, JSP/JSP fragments (.jsp/.jspf),
              SQL, HTML/XML/XAML/SVG/Svelte (template only), Lua, Dockerfile,
              MATLAB, Perl, PowerShell, Delphi/Object Pascal/Pascal, Modula-2,
              Fortran (free + fixed form), COBOL (free format), OCaml,
              Racket/Scheme/Common Lisp, Clojure, F#, Crystal, Nim, Zig,
              Verilog/SystemVerilog, VHDL, CoffeeScript, ColdFusion (tags only),
              Tcl, Elm, PureScript, Idris, Agda, Handlebars, Liquid, SAS,
              Wolfram Language, GAMS, Smalltalk, PL/I, Forth, Standard ML, x86
              Assembly, Logo, ABAP, Red, Io, Factor, J, APL, Sed, Stata, Vyper,
              Solidity, Move, Cairo, X10, Carbon, Ballerina, ReScript, Apex
              (trigger files only)
            ...
            JSP (.jsp/.jspf) strips <%-- --%> JSP comments, // and /* */ inside
            <% %>/<%! %>/<%= %>/<%@ %> tags (reusing the Java handler), and
            <!-- --> HTML comments in the template portion - all independently.
