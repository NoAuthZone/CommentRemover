...
By default, the original project is never modified - every operation runs on a copy (timestamped, or custom-named via `-o`), and symlinks that point outside the project are skipped rather than followed. An explicit `--in-place` flag is available for cases where you want to skip the copy and clean files directly.
...
- Reports a full lines-of-code (LoC) breakdown before/after - total, blank, comment-only, and pure code lines - both overall and per file type
...
- Displays changed file counts, added/removed line totals, overall size reduction, and LoC reduction
...
```bash
python3 CommentRemoverV2_2.py --path /path/to/project
```

Create a clean copy and a separate HTML diff report:
```bash
python3 CommentRemoverV2_2.py --path /path/to/project --debug-diff
```

Short path option:
```bash
python3 CommentRemoverV2_2.py -p /path/to/project --debug-diff
```

Use a custom name/location for the copy instead of the timestamped default:
```bash
python3 CommentRemoverV2_2.py -p /path/to/project -o /path/to/project-clean
```

Skip the copy entirely and clean the files directly in place (destructive - see [Important Notes](#important-notes)):
```bash
python3 CommentRemoverV2_2.py -p /path/to/project --in-place
```

Collapse blank lines left behind by removed comments:
```bash
python3 CommentRemoverV2_2.py -p /path/to/project --blank-lines squeeze
# or drop them entirely
python3 CommentRemoverV2_2.py -p /path/to/project --blank-lines drop
```

Disable the live progress indicator (useful for CI logs or piped output):
```bash
python3 CommentRemoverV2_2.py -p /path/to/project --no-progress
```

Show help:
```bash
python3 CommentRemoverV2_2.py --help
```

Show the version:
```bash
python3 CommentRemoverV2_2.py --version
```
...
```bash
python3 CommentRemoverV2_2.py -p /path/to/project -d
```

**`--del-include EXT ...`** - replaces the built-in list entirely; only the listed extensions are deleted:
```bash
python3 CommentRemoverV2_2.py -p /path/to/project -d --del-include .png .jpg .zip
```
...
```bash
python3 CommentRemoverV2_2.py -p /path/to/project -d --del-exclude .py .js .ts
```
...
- Removed and added line counts (diff-based - see [Lines of Code](#lines-of-code-loc) for how this differs from the LoC breakdown)
- Overall project size reduction
- Overall LoC (lines of code) reduction, plus a breakdown by file type
...
This is a different metric from the "Removed/Added lines" figures in the changed-files list and HTML report cards, which come from the Git-style diff and count *any* line whose content changed - including a line that just lost a trailing inline comment (`x = 1  # note` -> `x = 1`), which still counts as "Code" in the LoC breakdown since the line survives. So "Removed lines" (diff) is usually higher than "Comment-only lines" (LoC).
