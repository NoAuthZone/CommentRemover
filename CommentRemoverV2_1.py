...
def write_index(diff_dir, root, size_before=None, size_after=None):
    ...
    document = f'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>CommentRemover reports</title><style>{CSS}</style></head><body><main>{report_banner()}<h1>CommentRemover Debug Reports</h1><div class="metadata"><div class="metadata-row"><span class="metadata-label">Project:</span><span>{html.escape(root)}</span></div><div class="metadata-row"><span class="metadata-label">Generated:</span><span>{datetime.now():%Y-%m-%d %H:%M:%S}</span></div></div><div class="summary"><div class="card">Modified files<span class="value">{len(reports)}</span></div><div class="card">Removed lines<span class="value red">{removed}</span></div><div class="card">Added lines<span class="value green">{added}</span></div>{size_card}</div><section class="section"><h2 class="section-title">Directory structure and change status</h2>{filters}{tree_html(root)}</section><section class="section"><h2 class="section-title">Changed files</h2><ul class="report-list">{listing}</ul></section>{TREE_SCRIPT}</main></body></html>'''
...
def process_file(path, debug, diff_dir, root, blank_lines="keep"):
    ...
    cleaned_text = collapse_blank_lines(handler(original), blank_lines)
