...
def remove_lua_comments(text):
    """Remove Lua comments without damaging strings or long-bracket values.

    Supported comments:
        -- line comment
        --[[ block comment ]]
        --[=[ block comment ]=]  (arbitrary equals level)

    Preserved string content includes values such as "-- not a comment".
    """
    output = []
    index = 0
    length = len(text)
    quote = None
    escaped = False
    long_string_end = None

    def long_bracket_end(position):
    ...
    while index < length:
        ...
        if long_string_end is not None:
            if text.startswith(long_string_end, index):
                output.append(long_string_end)
                index += len(long_string_end)
                long_string_end = None
            else:
                output.append(char)
                index += 1
        ...
        bracket_end = long_bracket_end(index)
        if bracket_end is not None:
            opening_length = len(bracket_end)
            output.append(text[index:index + opening_length])
            index += opening_length
            long_string_end = bracket_end
            continue
        ...
        if text.startswith("--", index):
            ...
            # Long-bracket block comment, including --[[...]] and --[=[...]=].
            block_end = long_bracket_end(index)
            ...
            # Ordinary line comment. Preserve only the line ending.
...
def remove_php_comments(text):
    """Remove PHP comments and HTML comments without damaging PHP strings.

    HTML comment markers such as '<!--' and '-->' are valid string content in
    PHP libraries such as Parsedown. HTML comments are therefore removed only
    while the scanner is outside a PHP block.
    """
    output = []
    index = 0
    in_php = False
    ...
    while index < length:
        # Enter PHP mode. This also supports short echo tags such as <?=.
        ...
        # Outside PHP, remove only actual HTML comments.
        if not in_php:
            if text.startswith("<!--", index):
                index += 4
                while index < length:
        ...
        # Leave PHP mode only outside strings and comments.
        if pair == "?>":
            in_php = False
            output.append(pair)
            index += 2
        ...
        # PHP block comments while preserving their line breaks.
        if pair == "/*":
            index += 2
            while index < length:
                if text.startswith("*/", index):
...
def make_report(path, original, cleaned, output, root, compared=None, source_encoding="unknown", raw_size=None):
    ...
    source_bytes = raw_size if raw_size is not None else max(len(original.encode("utf-8")), len(cleaned.encode("utf-8")))
    ...
    document = (
        ...
        f'<div class="metadata-row"><span class="metadata-label">Source encoding:</span><span>{html.escape(source_encoding)}</span></div>'
...
def make_index(output, root):
    ...
    for number, report in enumerate(reports, 1):
        ...
        encoding_label = html.escape(str(report.get("encoding", "unknown")))
        rows.append(
            f'<li class="report-item"><span class="report-number">#{number}</span>'
            f'{source_view}<span><span class="status-removed">-{report["removed"]}</span> / '
            f'<span class="status-added">+{report["added"]}</span> · {encoding_label}</span></li>'
        )
...
def detect_newline(text):
    """Return the dominant newline sequence used by a source file."""
...
def apply_newline_style(text, newline):
    """Apply the original newline style to generated text."""
...
def looks_binary(data):
    """Return True for likely binary data while allowing Unicode text."""
    if not data:
        return False
    ...
    sample = data[:8192]
    if b"\x00" in sample:
        # UTF-16/UTF-32 without BOM generally has regular NUL positions.
        even_nuls = sample[0::2].count(0)
        odd_nuls = sample[1::2].count(0)
        pairs = max(1, len(sample) // 2)
        if max(even_nuls, odd_nuls) / pairs > 0.30:
            return False
        return True
...
def fallback_single_byte_encoding(data):
    """Select CP1252 only when CP1252-specific bytes are present."""
    cp1252_only = set(range(0x80, 0xA0))
    if any(byte in cp1252_only for byte in data):
        return "cp1252"
    return "latin-1"


def decode_source_file(path):
    """Read source text with BOM detection and strict encoding fallbacks.

    Returns:
        text, encoding, newline_style, original_byte_size
    """
    with open(path, "rb") as file:
        data = file.read()

    if looks_binary(data):
        raise UnicodeError("binary content detected")

    if data.startswith(b"\xef\xbb\xbf"):
        candidates = ["utf-8-sig"]
    elif data.startswith(b"\xff\xfe\x00\x00"):
        candidates = ["utf-32-le-bom"]
    elif data.startswith(b"\x00\x00\xfe\xff"):
        candidates = ["utf-32-be-bom"]
    elif data.startswith(b"\xff\xfe"):
        candidates = ["utf-16-le-bom"]
    elif data.startswith(b"\xfe\xff"):
        candidates = ["utf-16-be-bom"]
    else:
        sample = data[:8192]
        even_nuls = sample[0::2].count(0)
        odd_nuls = sample[1::2].count(0)
        ...
        if odd_nuls / pairs > 0.30:
            candidates = ["utf-16-le"]
        elif even_nuls / pairs > 0.30:
            candidates = ["utf-16-be"]
        else:
            candidates = ["utf-8", fallback_single_byte_encoding(data)]
    ...
    for encoding in candidates:
        try:
            ...
            elif encoding == "utf-32-le-bom":
                text = data[4:].decode("utf-32-le", errors="strict")
            elif encoding == "utf-32-be-bom":
                text = data[4:].decode("utf-32-be", errors="strict")
            else:
    ...
    raise UnicodeError(f"unable to decode file: {last_error}")


def encode_source_text(text, encoding):
    """Encode cleaned text with the original source encoding and byte order."""
    if encoding == "utf-16-le-bom":
    ...
    if encoding == "utf-32-le-bom":
        return b"\xff\xfe\x00\x00" + text.encode("utf-32-le", errors="strict")
    if encoding == "utf-32-be-bom":
        return b"\x00\x00\xfe\xff" + text.encode("utf-32-be", errors="strict")
...
def process_file(path, debug, diff_dir, root):
    ...
    handler = (
        ...
        else HANDLERS.get(os.path.splitext(filename)[1].lower())
    )
    if not handler:
        return

    try:
        original, source_encoding, newline_style, raw_size = decode_source_file(path)
    except (OSError, UnicodeError) as error:
        print(f"WARNING: Could not decode: {path}\n         {error}")
        return

    cleaned = restore_eof(original, handler(original))
    cleaned = apply_newline_style(cleaned, newline_style)
    ...
    if debug:
        ...
        try:
            ...
            report_error = None
        except (OSError, MemoryError, ValueError, UnicodeError) as error:
            report = None
    ...
    try:
        encoded = encode_source_text(cleaned, source_encoding)
